...
import { view, update, create, remove } from 'context/master-perkuliahan/action'
...
export default function ViewModal({ openModal, onClose, newEntry, onViewData, onUpdateSuccess }) {
  ...
  const [perkuliahan, dispatch] = useStateValue('masterPerkuliahan')
  const [errLoadingMataKuliah, loadingMataKuliah] = useStateDefault('PERKULIAHAN')
  ...
  React.useEffect(() => {
    ...
    if (openModal && prevOpenModal !== openModal) {
      setOpenModalVisible(true)
      view(dispatch, { id: onViewData._id || perkuliahan._id })
    }
    if (
      ...
      if (perkuliahan && perkuliahan.message) {
        message.success(perkuliahan.message)
      }
      setIsupdated(false)
      onUpdateSuccess()
    }
  ...
  const handleSave = () => {
    if (typeform === 'create') {
      ...
      update(dispatch, updatedData, { id: perkuliahan.data._id })
    }
  }
  const handleDelete = () => {
  ...
  const perkuliahandata =
    onViewData.deskripsiPerkuliahan || (perkuliahan && perkuliahan.data && perkuliahan.data.deskripsiPerkuliahan)
  const titles = perkuliahandata && typeform === 'edit' ? perkuliahandata : 'Perkuliahan'
  const ConfirmDelete = () => (
  ...
  return (
    ...
    >
      {edit ? (
        <FormInput returnData={setInputData} type={typeform} payload={perkuliahan ? perkuliahan.data : {}} />
      ) : perkuliahan && perkuliahan.data && !loadingMataKuliah ? (
        <div className="view">
          {objectToArray(perkuliahan.data)
