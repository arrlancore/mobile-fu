...
export const actionTypes = {
  ...
  JAM_PERKULIAHAN_SUCCESS: 'JAM_PERKULIAHAN_SUCCESS',
  LIST_JAM_PERKULIAHAN: 'LIST_JAM_PERKULIAHAN',
  LIST_JAM_PERKULIAHAN_SUCCESS: 'LIST_JAM_PERKULIAHAN_SUCCESS'
}
 ...
 * *** MASTER JAM_PERKULIAHAN ***
 */

const moduleRoutes = '/api/master-jam-perkuliahan'
...
export const list = (dispatch, params) => {
  ...
  const action = async () => {
    ...
    if (response.status <= 204) {
      ...
      dispatch({
        type: actionTypes.LIST_JAM_PERKULIAHAN_SUCCESS,
        data
      })
    } else {
  ...
  dispatchAction(dispatch, actionTypes.LIST_JAM_PERKULIAHAN, action)
}
...
export const view = (dispatch, params) => {
  ...
  const action = async () => {
    ...
    if (response.status <= 204) {
      let data = response.data
      dispatch({
        type: actionTypes.JAM_PERKULIAHAN_SUCCESS,
        data
      })
    } else {
      const message = response.data && response.data.message
  ...
  dispatchAction(dispatch, actionTypes.JAM_PERKULIAHAN, action)
}
